...
A picker can implement `pick_with_cost()` to return a row and an optional finite cost for host diagnostics. Existing `pick()` implementations remain valid. The host checks the row and cost before booking. `with_exclusive_affinity(true)` asks the host to constrain candidates to an eligible session-affinity target; explicit request pins remain mandatory for every policy.
