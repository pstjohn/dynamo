...
This feature replaces the worker-ranking part of Dynamo's routing pipeline. A `WorkerFilter` can exclude a host-eligible worker, a `WorkerScorer` assigns a finite cost to each remaining worker, Dynamo adds costs from all configured scorers, and a `WorkerPicker` chooses one row from the scored candidates. Filters run in declaration order. Each scorer can then prepare an aggregate from all surviving workers before scoring each worker. Rejecting every worker returns an error. Lower costs rank first by convention, but the picker can implement deterministic selection, sampling, tie-breaking, or policy-local state. Dynamo continues to own discovery, eligibility, score validation, accounting, and reservation.
...
flowchart LR
    ...
    Filters --> Prepare["WorkerScorer.prepare<br/>all surviving workers"]
    Prepare --> Scorers["WorkerScorer.score<br/>one cost per worker"]
...
| `WorkerScorer` | Request context; all surviving workers in `prepare`, one worker in `score` | One finite cost per worker | The rule ranks workers or adds a penalty |
...
An external policy owns its filters, scorers, and picker. The default scorer and picker are private components of `dynamo-custom-policy-builtin`. They implement the same public traits as external policies.
...
| `plugins::request_classifier` | `RequestClassifier`, `ClassifyRequest`, `ClassifyEvent`, configuration, and factories |
...
This scorer first finds the lowest active-request count among surviving workers, then scores each worker by the number of requests above that minimum. Subtracting the same minimum from each cost preserves the ranking:
...
`prepare` is optional and defaults to no work. Dynamo calls it once per scorer after filtering, before any `score` call. It receives a borrowed slice with the same declared inputs and snapshot used for scoring. Recompute request-local aggregates on each call. The slice order is unspecified, and the slice cannot be retained. Dynamo skips preparation when no candidates survive; a preparation error stops selection before picking or booking.
...
fn provider(
    ...
    Ok(Arc::new(move |config, worker_type, _partition| {
        ...
        let scorers: Vec<Box<dyn WorkerScorer>> = vec![Box::new(ActiveRequestsScorer::default())];
...
flowchart LR
    ...
    Build --> Classifier[Per-router request classifier]
```
...
| Accessor | Meaning |
|---|---|
| `prompt_tokens()` | Exact incoming prompt token count, borrowed from the request without allocation |
| `request_blocks()` | Incoming prompt size in KV blocks, rounded up |
| `has_tier_matches()` | Whether the current request lookup contains any tier-specific matches, before policy filters |
| `shared_cache_hits()` | Optional unweighted shared-cache ranges from the current lookup |
...
- Keep blocking I/O and panics out of `keep`, `prepare`, `score`, and `pick`.
...
The default worker selector lives in `dynamo-custom-policy-builtin`, beside the two-tier policy. Routing hosts install `default_registry()` even when they do not link a custom catalog. An omitted policy or the reserved `default` selection uses that factory. A Rust embedding that constructs `SelectionServiceBuilder` must supply a registry with a default factory or an explicit policy factory.

The default uses the same `RouterPluginRegistry` as custom worker policies and request classifiers. Installing it preserves the other registered providers. Resolving a classifier-only configuration still leaves worker selection on the builtin default; installing the default alone does not enable custom-plugin frontend restrictions.

To configure a separate default-policy instance, use the named provider:
...
The provider also accepts `overlap_score_credit_decay`, `decode_active_request_weight`, `host_cache_hit_weight`, `disk_cache_hit_weight`, and `shared_cache_multiplier`. Omitted parameters inherit router configuration. Values must be finite and non-negative. The default plugin uses configured parameters and does not apply per-request score or temperature overrides. Plain disaggregated decode remains load-only when prefill tracking is off and conditional disaggregation is disabled.

`WorkerCacheInput::accounting_cache_estimate()` returns the host's weighted block estimate and rounded token estimate. Missing estimates are zero. These estimates are not clamped to prompt length and do not measure physical GPU residency. The plugin can use them without changing the load that the host records. `WorkerLoadInput::is_available()` distinguishes a missing load observation from an observed idle worker. Both values describe the current selection snapshot.

The default scorer owns the cache and load formula, including preferred-taint costs. Its `prepare` method finds the minimum prefill load used to reduce cache credit on busier workers. The picker reads only the final costs and owns temperature sampling and tie-breaking. The host checks scorer costs and the selected row before booking; diagnostics use the selected row's summed scorer cost. `with_exclusive_affinity(true)` asks the host to constrain candidates to an eligible session-affinity target; explicit request pins remain mandatory for every policy.

Direct Rust callers and replay import `DefaultWorkerSelector` from `dynamo_custom_policy_builtin`. Scheduler actors use `default_policy()` directly. The core's old selector is available only as a test and benchmark reference, not in production builds.

The builtin crate keeps composition in `default/mod.rs`, the cost formula in `default/scorer.rs`, selection in `default/picker.rs`, startup parameters in `default/parameters.rs`, and the synchronous adapter in `default/selector.rs`. The two-tier policy remains in its own module. Preparation reads the existing snapshot; it adds no tracker synchronization. The routing core still projects load, selects and books serially, and processes request lifecycle updates.
